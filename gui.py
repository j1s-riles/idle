...
def buy_button(slimetype:slime_type, amt: int, lbl:ttk.Label, btn:ttk.Button):
    ...
    lbl["text"] = floor(app.slimes[slimetype].quantity)
...
btn_gen2_buy = ttk.Button(window, text=app.slimes[slime_type.RED].price_check(1))
btn_gen2_buy["command"] = partial(buy_button, slime_type.RED, 1, lbl_gen2_amt, btn_gen2_buy)

lbl_gen2.grid(row=2, column=0)
lbl_gen2_amt.grid(row=2, column=1)
btn_gen2_buy.grid(row=2, column=2)
